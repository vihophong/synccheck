...
void Spy(std::vector<uint64_t> & v,std::vector<DataStruct> & e,int & vid,const int id){
  ...
  while(1){
        ...
        int x=0,k=0;
        uint32_t buffer32 [16*1024];
        memset(buffer32,0,16*1024);
       ...
       while (x==0){
               ...
               int i;
                if(x>0){
					...
					if(debn<5)std::cerr<<"Id "<<std::dec<< id<<" TimeStamp 0d"<<std::dec<<TimeStamp<<std::endl;
					debn++;
#endif
                ...
                if(k>50)x=1; // Wait for 5 seconds for data
        }
        dataSpyClose (id);
...
MyMainFrame::MyMainFrame(const TGWindow *p, UInt_t w, UInt_t h)
{
    ...
    double ul2d= 10e3;
   	...
   	if(Master==0){ 
    		h2=new TH2F("Check","Synchronization Check;RIBF-EURICA time difference        x40ns;AIDA-EURICA time difference                  x40ns",200, ll2d, ul2d, 200, ll2d, ul2d); 
    		h2partial=new TH2F("CheckPartial","Partial;RBIF-EURICA time difference        x40ns;AIDA-EURICA time difference                  x40ns",200, ll2d, ul2d, 200, ll2d, ul2d);

    		h1X=new TH1F("Check","RIBF-EURICA time difference;                                x40ns",4000, ll1d, ul1d);
    		h1Y=new TH1F("Check","AIDA-EURICA time difference;                                x40ns",4000, ll1d, ul1d);
	}
	        else if(Master==1){
                h2=new TH2F("Check","Synchronization Check;AIDA-RIBF time difference        x40ns;EURICA-RIBF time difference                  x40ns",200, ll2d, ul2d, 200, ll2d, ul2d);
                h2partial=new TH2F("CheckPartial","Partial;AIDA-RIBF time difference        x40ns;EURICA-RIBF time difference                  x40ns",200, ll2d, ul2d, 200, ll2d, ul2d);

                h1X=new TH1F("Check","AIDA-RIBF time difference;                                x40ns",4000, ll1d, ul1d);
                h1Y=new TH1F("Check","EURICA-RIBF time difference;                                x40ns",4000, ll1d, ul1d);
        }
        else if(Master==2){
                h2=new TH2F("Check","Synchronization Check;RIBF-AIDA  time difference        x40ns;EURICA-AIDA  time difference                  x40ns",200, ll2d, ul2d, 200, ll2d, ul2d);
                h2partial=new TH2F("CheckPartial","Partial;RIBF-AIDA  time difference        x40ns;EURICA-AIDA  time difference                  x40ns",200, ll2d, ul2d, 200, ll2d, ul2d);

                h1X=new TH1F("Check","EURICA-AIDA time difference;                                x40ns",4000, ll1d, ul1d);
