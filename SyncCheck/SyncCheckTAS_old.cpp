...
//x= dataSpyRead (id, (char*)buffer32,BUFFERSIZE);
       ...
       //	  if(id==0)  x= MyDataSink.Spy( buffer32);
               int i;
 	//std::cerr<<"kk1 en.. Id "<<std::dec<< id<<std::endl;
                 if(x>0){ 
	mtx.lock();
...
int dispcount=0;
 ...
 //                                       Data.E=buffer32[i+1];
                                        ...
                                        e.push_back(Data);
		std::cout<<"Id "<<std::dec<< id<<" buff0 0x"<<std::hex<<buffer32[i+0]<<" "<<"buff1 0x"<<std::hex<<buffer32[i+1]<<" "<<"buff2 0x"<<std::hex<<buffer32[i+2]<<" "<<"buff3 0x"<<std::hex<<buffer32[i+3]<<" "<<std::endl;
		std::cerr<<"Id "<<std::dec<< id<<" buff0 0x"<<std::hex<<buffer32[i+0]<<" "<<"buff1 0x"<<std::hex<<buffer32[i+1]<<" "<<"buff2 0x"<<std::hex<<buffer32[i+2]<<" "<<"buff3 0x"<<std::hex<<buffer32[i+3]<<" "<<std::endl;
//				        std::cerr<<"Id "<<std::dec<< id<<" TimeStamp 0x"<<std::hex<<TimeStamp<<" ";
//					dispcount++;
//					if(dispcount>3){std::cerr<<std::endl; dispcount=0;}
...
int vsize=0;
        ...
        for(j2=0;j2<3;j2++){
				...
				if(MergeMap[j2].find(TSvect[j2][k])!=MergeMap[j2].end())std::cerr<< j2 << " Data already exist" <<std::endl;
...
std::cout<< "0000000000000000000000000000000000000000" <<std::endl;
	...
	//	std::cout<< j2 <<" TS 0x "<<std::hex<<itx.first <<std::endl;
		...
		std::cout<< j2 <<" ------------------> Erase TS 0x"<<std::hex<<itx.first <<std::endl;
                  ...
                  else if((itx.second.Mark&0xf)==1){   // Erase element already used
		std::cout<< j2 <<" ------------------> Erase TS 0x"<<std::hex<<itx.first <<std::endl;
...
//                  for (auto& itx: MergeMap){
	...
	for(auto itx:MergeMap[Master]){
					...
					std::cerr<<"Find Coinc 1 "<<std::dec<< Diff<< "Mark"
						<< it->second.Mark << std::hex <<" Main TS 0x"
						<<itx.first << " 1 TS 0x" << it->first <<std::endl;
						...
						<< std::hex <<" Main TS 0x"<<itx.first << " 2 TS 0x" << it->first <<std::endl;
                                        if((it->second.Mark&0xf)==0){
        					mtx.lock();
              						Y.push_back(Diff);
							X.push_back(0xFFFF);
...
MyMainFrame::MyMainFrame(const TGWindow *p, UInt_t w, UInt_t h)
{
    ...
    double ul2d= UL2D;
   	...
   	if(Master==0){ 
    		...
    		h1X=new TH1F("Check","RIBF-DTAS time difference;                                x40ns",200, ll1d, ul1d);
    		h1Y=new TH1F("Check","AIDA/WASABI-DTAS time difference;                                x40ns",200, ll1d, ul1d);
	}
	        else if(Master==1){
                ...
                h1Y=new TH1F("Check","DTAS-RIBF time difference;                                x40ns",200, ll1d, ul1d);
        }
        else if(Master==2){
