...
{
    if((INTS4)*pl_histos < l_histos) {l_status=COMM__ERROR;goto g_return;}
    pl_all = *p_buffer;
}
*pl_histos = (INTS4 *)l_histos;
l_buffer = s_his_comm_cli.lu_size;
/* break total buffer in TCP buffers */
pl_l=pl_all;
i_l=l_buffer/16384;
...
{
    if((INTS4)*pl_size < l_size) {l_status=COMM__ERROR;goto g_return;}
    pl_all = *p_buffer;
}
