  ...
  INTS4 l_swap, l_swap_head, l_is_goosybuf, l_filehead=0, l_size, l_size_head, l_dummy,l_header_size;
   ...
   case GETEVT__FILE :
	...
	fLmdGetOpen(ps_chan->pLmd,c_file,NULL,LMD__BUFFER,LMD__NO_INDEX);
	  ...
	  printf("check header: swap=%d, header=%d, buffer=%d, size=%d\n",
             l_swap_head,l_filehead,l_is_goosybuf,l_size_head);
      if(((l_is_goosybuf==0)&(l_filehead==0))|(l_size_head==0))
	{
	  printf("error swap=%d, header=%d, buffer=%d, size=%d\n",
             l_swap_head,l_filehead,l_is_goosybuf,l_size_head);
	  close(ps_chan->l_channel_no);
	  ps_chan->l_channel_no=-1;
          return(GETEVT__RDERR);
        ...
        if(read(ps_chan->l_channel_no,c_temp,MIN_BUF_LGTH)!=MIN_BUF_LGTH)
	{
	  ...
	  printf("Check buffer: swap=%d, header=%d, buffer=%d, size=%d\n",
        ...
        if((l_is_goosybuf==0)|(l_size!=l_size_head)|(l_swap!=l_swap_head))
	{
      ...
      if(l_filehead == 1)
      {
	printf("Read header again, %d bytes\n",l_header_size);
         if(read(ps_chan->l_channel_no,c_temp,l_header_size)!=l_header_size)
         {
	    close(ps_chan->l_channel_no);
	    ps_chan->l_channel_no=-1;
   ...
   case GETEVT__TRANS  :
	...
	fLmdInitMbs(ps_chan->pLmd,pc_server,ps_chan->l_buf_size,ps_chan->l_bufs_in_stream,0);
        ps_chan->l_server_type=l_mode;
        return GETEVT__SUCCESS;
      }
#endif
  ...
  INTS4 l_temp,l_prev_ok=1,l_stat,l_used;
   ...
   if(ps_chan->pLmd != NULL){
     if(ps_chan->l_server_type == GETEVT__TRANS)
       ps_ve10_1=(s_ve10_1 *)fLmdGetMbsEvent(ps_chan->pLmd);
     if(ps_chan->l_server_type == GETEVT__FILE)
       ps_ve10_1=(s_ve10_1 *)fLmdGetElement(ps_chan->pLmd,LMD__NO_INDEX);
     if(ps_ve10_1 == NULL) return(GETEVT__RDERR);
     if(ppl_goobuf)*ppl_goobuf = NULL;
     *ppl_buffer = (INTS4 *)ps_ve10_1;
     return(GETEVT__SUCCESS);
   }
#endif
...
/* above, if this i/o buffer is read to end */
/* above, end of this read_buffer which may contain several GOOSY buffers*/
         {
         ...
         if(ps_chan->ps_bufhe->i_type==2000)
         {   /* file header */
      ...
      if( (ps_chan->l_evt_buf_posi!=0)||( (ps_chan->ps_bufhe->h_end==1)&&(ps_chan->l_first_buf==1) ) )
      {      /* if this is a spanned part of an event */
...
INTS4 f_evt_get_close(s_evt_channel * ps_chan)
{
   ...
   if(ps_chan->pLmd != NULL){
     ...
     if(ps_chan->l_server_type == GETEVT__FILE) fLmdGetClose(ps_chan->pLmd);
     free(ps_chan->pLmd);
     ps_chan->pLmd=NULL;
     return GETEVT__SUCCESS;
   }
      ...
      INTS4 l_type, INTS4 l_subtype, s_evt_channel *ps_chan, CHARS *ps_filhe)
{
   ...
   if(l_stream==0) {
     ps_chan->pLmd=fLmdAllocateControl();
     if(ps_filhe != NULL){
       ((s_filhe *)ps_filhe)->filhe_free[2]=158+((s_filhe *)ps_filhe)->filhe_lines*40; // words
     }
     l_status=fLmdPutOpen(ps_chan->pLmd,pc_file,(sMbsFileHeader *)ps_filhe,l_size,
