...
INTS4 f_ut_status_ini(s_daqst *ps_daqst)
{
 ...
 ps_daqst->l_fix_lw            = ((int)&ps_daqst->c_pname[0][0]-(int)ps_daqst)/4;
...
INTS4 f_ut_status_r(s_daqst *ps_daqst, INTS4 l_tcp)
{
  ...
  if(ps_daqst->l_version == 3)
    {
      l_status = f_stc_read (&ps_daqst->bh_daqst_initalized, (ps_daqst->l_fix_lw-7)*4 , l_tcp,-1);
      if(l_swap == 1) l_status = f_swaplw(&ps_daqst->bh_daqst_initalized, (ps_daqst->l_fix_lw-7) - (15 * SBS__STR_LEN_64/4)-256,NULL);
    }
...
ps_setup->l_fix_lw     = ((int)&ps_setup->lp_rem_mem_base[0]-(int)ps_setup)/4;
...
ps_set_ml->l_fix_lw        = ((int)&ps_set_ml->c_rd_hostname[0][0]-(int)ps_set_ml)/4;
...
ps_set_mo->l_swap_lw       = ((int)&ps_set_mo->c_ds_hostname[0][0]-(int)ps_set_mo)/4;
