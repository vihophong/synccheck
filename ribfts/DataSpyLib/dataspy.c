...
/*    attach the memory segment */
  ...
  fprintf(stderr, "dataSpy Shared buffer area %d (/SHM_%d) located at 0x%x\n", id, shmkey+id,shm_bufferarea[id]);
  ...
  fprintf(stderr, "dataSpy Current age %lld index %d\n", current_age[id],next_index[id]);
...
/*    detach the memory segment */
  ...
  fprintf(stderr, "dataSpy Shared buffer area %d located at 0x%x detached\n",id,shm_bufferarea[id]);
...
int dataSpyReadWithSeq (int id, char *data, int length, int *seq)
{
 ...
 retry:
        ...
        if(baseaddress->buffer_age[next_index[id]] >= current_age[id])
	{
	   ...
	   if(verbose)
	      fprintf(stderr, "dataSpyRead id %d: Age %lld Index %d Buffer length %d\n",
...
/*    check if the entry could have changed while copying (can happen) and if so retry   */
	   ...
	   {
	        if(verbose)
	            fprintf(stderr, "dataSpyRead id %d: Copied oldage %lld newage %lld\n", id, current_age[id], baseaddress->buffer_age[next_index[id]]);
    ...
    {
       if(verbose)
          fprintf(stderr, "dataSpyRead - id %d has no data\n",id);
    }

    if(verbose)
       fprintf(stderr, "dataSpyRead - id %d length %d\n",id,len);
